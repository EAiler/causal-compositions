...
def plot_mse_results(df_mse,
    ...
    if sort_to_filter:
        ...
        sorter = filter_list
        df_mse_mask["Method"] = df_mse_mask.Method.cat.set_categories(sorter)
...
def plot_beta_results(df_beta, betaT,
    ...
    """
    unique_beta = np.round(np.unique(np.round(betaT, 4)), 3).tolist()
    betaT = onp.array(np.round(betaT, 3))
    ...
    fig = go.Figure()
    if beta_zero:
        num_coef_list = set((onp.abs(betaT) < 0.09) * onp.array(range(len(betaT)))) - {0}
    else:
        num_coef_list = set((onp.abs(betaT) > 0.09) * onp.array(range(len(betaT))))
    ...
    if sort_to_filter:
        ...
        sorter = filter_list
        df_beta_mask["Method"] = df_beta_mask.Method.cat.set_categories(sorter)
...
def plot_diversity_methods(x, y, xstar, xstar_bound, ystar_ols, ystar_2sls, ystar_kiv, results, ytrue=None, z=None):
    """
