...
def make_cholesky_factor(l_param: np.ndarray) -> np.ndarray:
  ...
  lmask = lmask.at[0,0].set(0) #index_update(lmask, (0, 0), 0)
  tmp = l_param * lmask
  idx = np.diag_indices(l_param.shape[0])
  return tmp.at[idx].set(np.exp(tmp[idx]))
...
def fit_bounds(z: np.ndarray,
  ...
  for i, xstar in enumerate(xstar_grid):
    for j, bound in enumerate(["lower", "upper"]):
      ...
      final["indices"] = final["indices"].at[i, j].set(fin_i)      
      final["objective"] = final["objective"].at[i, j].set(fin_obj)
      final["maxabsdiff"] = final["maxabsdiff"].at[i, j].set(fin_diff)
      satis["indices"] = satis["indices"].at[i,j].set(sat_i)
      satis["objective"] = satis["objective"].at[i,j].set(sat_obj)
      satis["maxabsdiff"] = satis["maxabsdiff"].at[i,j].set(sat_diff)
